        ...
        .then(function(res) {
          ...
          queue.push(urllib.request, url + '/' + name, function(err, result) {
            ...
            if (flag && record) {
              ...
              var err = record > 1 ? ' errors were' : ' error was';
              var autoTips = options.update ? '' : chalk.red(' , please run') + chalk.bold(' `modules-version-check udpate`') + chalk.red(' to update. ');
              console.log('');
              console.log(
                emoji.get(':warning:'),
              ...
              console.log('');
              console.log(emoji.get(':package:'), ' Installing... (If an error occurred during the automatic installation, try to run `npm install` manually for installation)');
