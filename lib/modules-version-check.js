...
function modulesVersionCheck(options, callback) {
  ...
  forIn(dependencies, (value, name) => {
    ((name, version) => {
      co(function* () {
        let localPkg;
        let result = yield urllib.requestThunk(REGISTRY_URL.npmRegistry + '/' + name, {timeout: 150000});
        ...
        if (verifyMain(result.version, matchMaxVersion, version, localPkg.version)) {
          ...
          if (cli == 'npm') {
            ...
            if (localPkg.version === '*') {   // any version
              nameList.push(name + '@' + localPkg.version);
            } else {
              nameList.push(name + '@^' + matchMaxVersion);
            }
          }
        ...
        if (isFinish && record) {
          ...
          const commandStr = cli == 'npm' ? 'npm install ' : 'yarn add ';
          const commandOp = cli == 'npm' ? 'install' : 'add';
